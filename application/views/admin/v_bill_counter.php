              ...
              <div class="card-body" style="overflow-x:auto;">
                <table class="table table-hover table-bordered" id="sampleTable">
                  ...
                  <tbody>
                      ...
                      <?php 
                        if($row['status']==2) echo "<td>Đã thanh toán</td>";
                        else if($row['status']==1) echo '<td style="background-color: rgba(0, 0, 255, 0.25);"> Chưa thanh toán</td>';
                        else echo "<td style='background-color: rgba(255, 0, 0, 0.25);'>Chưa xác nhận</td>";
                      ?>
                      <td>
                  
                        <?php 
                        echo '<button class="btn btn-info btn-lg glyphicon glyphicon-ok" style="border-radius: 10px;" onclick="checkRow('. $row['id'] .')"></button>';
                        echo '<button class="btn btn-primary btn-lg glyphicon glyphicon-hand-right" style="border-radius: 10px;" onclick="nextRow('. $row['id'] .')"></button>';
                        echo '<button class="btn btn-warning btn-lg glyphicon glyphicon-hand-left" style="border-radius: 10px;" onclick="preRow('. $row['id'] .')"></button>';
                        ?>
                  	  </td>
                    </tr>
                  	<?php
...
<script type="text/javascript">
  ...
  function checkRow(id)
    {
        ssi_modal.confirm({
        content: 'Xác nhận đơn hàng này đã thanh toán và chuyển đến đơn vị giao hàng?',
        okBtn: {
        ...
        className:'btn btn-danger'
        ...
        },function (result) 
            {
                if(result)
                {
                    var route="<?= base_url()?>admin/admin/checkPay/";
...
{
    ssi_modal.confirm({
    content: 'Đơn hàng này chưa thanh toán và chuyển đến đơn vị giao hàng?',
    okBtn: {
    className:'btn btn-primary'
    },
    cancelBtn:{
    className:'btn btn-danger'
    ...
    },function (result) 
        {
            if(result)
            {
                var route="<?= base_url()?>admin/admin/checkNotPay/";
...
{
    ssi_modal.confirm({
    content: 'Đơn hàng này cần xác nhận lại?',
    okBtn: {
    className:'btn btn-primary'
    },
    cancelBtn:{
    className:'btn btn-danger'
    ...
    },function (result) 
        {
            if(result)
            {
                var route="<?= base_url()?>admin/admin/payBack/";
